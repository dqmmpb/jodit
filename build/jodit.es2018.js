...
class Uploader_Uploader extends Component_Component {
    ...
    sendFiles(insertImageAsLocalURI, files, handlerSuccess, handlerError, process) {
        if (!files) {
    ...
    bind(form, handlerSuccess, handlerError) {
        const self = this, onPaste = (e) => {
            ...
            if (cData && cData.files && cData.files.length) {
                this.sendFiles(insertImageAsLocalURI, cData.files, handlerSuccess, handlerError);
                return false;
            }
            if (browser('ff') || IS_IE) {
                if (cData &&
                    ...
                    this.jodit.async.setTimeout(() => {
                        ...
                        if (child && child.hasAttribute('src')) {
                            ...
                            restore();
                            self.sendFiles(insertImageAsLocalURI, [Uploader_Uploader.dataURItoBlob(src)], handlerSuccess, handlerError);
                        }
                    }, this.jodit.defaultTimeout);
                }
                return;
                    ...
                    if (items[i].kind === 'file' &&
                        ...
                        if (file) {
                            ...
                            extension = mime[1]
                                ? mime[1].toLowerCase()
                                : '';
                            this.sendFiles(insertImageAsLocalURI, [file], handlerSuccess, handlerError, process);
                        }
                        e.preventDefault();
                        break;
                    }
        ...
        self.jodit.events
            ...
            if (hasFiles(event) &&
                ...
                const insertImageAsLocalURI = inputUploadImageToCloud
                    ? !inputUploadImageToCloud.checked
                    : self.options.insertImageAsLocalURI;
                this.sendFiles(insertImageAsLocalURI, event.dataTransfer.files, handlerSuccess, handlerError);
            }
        });
        ...
        if (inputFile) {
            self.jodit.events.on(inputFile, 'change', function () {
                ...
                const insertImageAsLocalURI = inputUploadImageToCloud
                    ? !inputUploadImageToCloud.checked
                    : self.options.insertImageAsLocalURI;
                self.sendFiles(insertImageAsLocalURI, this.files, handlerSuccess, handlerError).then(() => {
                    inputFile.value = '';
