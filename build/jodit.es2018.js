...
var bundle = __webpack_require__(98);
...
const MAY_BE_REMOVED_WITH_KEY = /^(A|IMG|BR|IFRAME|SCRIPT|INPUT|TEXTAREA|HR|JODIT|JODIT-MEDIA)$/i;
...
var filesize = __webpack_require__(79);
...
(function (Widget) {
    ...
    Widget.FileSelectorWidget = (editor, callbacks, elm, close, isImage = true) => {
        ...
        if (callbacks.upload &&
            ...
            const dragbox = editor.create.fromHTML('<div class="jodit_draganddrop_wrapper">' +
...
function addNewLine(editor) {
    ...
    editor.events
        ...
        editor.events
            ...
            .on(editor.editor, 'mousemove', editor.async.debounce((e) => {
            ...
            const editorBound = offset_offset(editor.editor, editor, editor.editorDocument);
            const position = offset_offset(currentElement, editor, editor.editorDocument);
            let top = false;
...
class backspace_backspace extends Plugin_Plugin {
    ...
    onDelete(toLeft) {
        ...
        try {
            ...
            if (parent && fakeNode.nodeValue === EMPTY) {
    ...
    constructor() {
        ...
        this.addEvents = () => {
            ...
            editor.events
                ...
                'mousedown.placeholder afterSetMode.placeholder', this.toggle)
...
function resizer(editor) {
    ...
    const resizerElm = editor.create.fromHTML(`<div style="display:none" class="jodit_resizer">
        ...
        if (resizerIsVisible && currentElement && resizerElm) {
            const workplacePosition = offset_offset((resizerElm.parentNode ||
                editor.ownerDocument
                    .documentElement), editor, editor.ownerDocument, true), pos = offset_offset(currentElement, editor, editor.editorDocument), left = parseInt(resizerElm.style.left || '0', 10), top = parseInt(resizerElm.style.top || '0', 10), w = resizerElm.offsetWidth, h = resizerElm.offsetHeight;
    ...
    editor.events
        ...
        .on('change afterInit afterSetMode', editor.async.debounce(() => {
        if (resizerIsVisible) {
            if (!currentElement || !currentElement.parentNode) {
                hideResizer();
            }
...
function size_size(editor) {
    ...
    if (editor.options.height !== 'auto' &&
        ...
        editor.events
            ...
            .on(editor.ownerWindow, 'mousemove touchmove', editor.async.throttle((e) => {
            if (isResized) {
...
class Uploader_Uploader extends Component_Component {
    ...
    sendFiles(insertImageAsBase64URI, files, handlerSuccess, handlerError, process) {
        if (!files) {
    ...
    bind(form, handlerSuccess, handlerError) {
        const self = this, onPaste = (e) => {
            ...
            const inputUploadImageToCloud = form.querySelector('input[name=uploadImageToCloud]');
            ...
            if (cData && cData.files && cData.files.length) {
                this.sendFiles(insertImageAsBase64URI, cData.files, handlerSuccess, handlerError);
                return false;
            }
            if (browser('ff') || IS_IE) {
                if (cData &&
                    ...
                    this.jodit.async.setTimeout(() => {
                        ...
                        if (child && child.hasAttribute('src')) {
                            ...
                            restore();
                            self.sendFiles(insertImageAsBase64URI, [Uploader_Uploader.dataURItoBlob(src)], handlerSuccess, handlerError);
                        }
                    }, this.jodit.defaultTimeout);
                }
                return;
                    ...
                    if (items[i].kind === 'file' &&
                        ...
                        if (file) {
                            ...
                            extension = mime[1]
                                ? mime[1].toLowerCase()
                                : '';
                            this.sendFiles(insertImageAsBase64URI, [file], handlerSuccess, handlerError, process);
                        }
                        e.preventDefault();
                        break;
                    }
        ...
        self.jodit.events
            ...
            if (hasFiles(event) &&
                ...
                const insertImageAsBase64URI = inputUploadImageToCloud
                    ? !inputUploadImageToCloud.checked
                    : self.options.insertImageAsBase64URI;
                this.sendFiles(insertImageAsBase64URI, event.dataTransfer.files, handlerSuccess, handlerError);
            }
        });
        ...
        if (inputFile) {
            self.jodit.events.on(inputFile, 'change', function () {
                ...
                const insertImageAsBase64URI = inputUploadImageToCloud
                    ? !inputUploadImageToCloud.checked
                    : self.options.insertImageAsBase64URI;
                self.sendFiles(insertImageAsBase64URI, this.files, handlerSuccess, handlerError).then(() => {
                    inputFile.value = '';
